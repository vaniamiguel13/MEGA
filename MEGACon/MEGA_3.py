...
def MEGAcon(Problem, InitialPopulation, Options, *varargs):
    DefaultOpt = {
        ...
        'CPTolerance': 1.0e-6,
        'CPGenTest': 0.01,
        'CTol': 1e-2,  # Keeping the constraint tolerance the same
        'CeqTol': 1e6,  # Set CeqTol to 1e6
        'NormType': np.inf,
        'NormCons': 1,
        'Verbosity': 0
    }
    ...
    while Problem['Stats']['GenCounter'] < MaxGenerations and Problem['Stats']['ObjFunCounter'] < MaxEvals:
        ...
        if Conflag:
            max_c = np.max(np.abs(Population['c']), axis=0)
            max_ceq = np.max(np.abs(Population['ceq']), axis=0)
            for i in range(Pop - int(pool), Pop):
                if NormCons:
                    ...
                    maxceq[maxceq == 0] = 1
                    Population['Feasible'][i] = (norm(np.maximum(0, Population['c'][i, :]) / maxc,
                                                                NormType) <= CTol) and (norm(
                        np.abs(Population['ceq'][i, :]) / np.abs(maxceq), NormType) <= CeqTol)
                else:
                    Population['Feasible'][i] = (
                        norm(np.maximum(0, Population['c'][i, :]) / max_c, NormType) <= CTol and
                        norm(np.abs(Population['ceq'][i, :]) / max_ceq, NormType) <= CeqTol
                    )
