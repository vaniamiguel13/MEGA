...
def MEGAcon(Problem, InitialPopulation, Options, *varargs):
    ...
    if 'LB' not in Problem or 'UB' not in Problem or not isinstance(Problem['LB'], np.ndarray) or not isinstance(Problem['UB'], np.ndarray):
    ...
    Problem, Population = init_population(Problem, InitialPopulation, Pop, Conflag, CTol, CeqTol, NormType, NormCons, *varargs)

    # Check for feasible solutions in the initial population
    if np.sum(Population['Feasible']) == 0:
        print("Initial Population:", Population['x'])
        print("Constraints (c):", Population['c'])
        print("Constraints (ceq):", Population['ceq'])
        raise ValueError('No feasible solutions found in the initial population.')
            ...
            for i in range(Pop - int(pool), Pop):
                if NormCons:
                    ...
                    Population['Feasible'][i] = (np.linalg.norm(np.maximum(0, Population['c'][i, :]) / maxc,
                                                                NormType) <= CTol) and (np.linalg.norm(
                        np.abs(Population['ceq'][i, :]) / np.abs(maxceq), NormType) <= CeqTol)
                else:
                    Population['Feasible'][i] = (np.linalg.norm(np.maximum(0, Population['c'][i, :]),
                                                                NormType) <= CTol) and (
                                                            np.linalg.norm(np.abs(Population['ceq'][i, :]),
                                                                           NormType) <= CeqTol)
...
def init_population(Problem, InitialPopulation, Size, Conflag, CTol, CeqTol, NormType, NormCons, *varargs):
    c_dim = Problem['c_dim']
    ceq_dim = Problem['ceq_dim']

    Population = {
        'x': np.zeros((Size, Problem['Variables'])),
        'f': np.zeros((Size, Problem['Objectives'])),
        'c': np.zeros((Size, c_dim)),
        'ceq': np.zeros((Size, ceq_dim)),
...
def rank_population(Population, elite, sigma, NormType):
    ...
    for i in IP:
        ...
        Population['Fitness'][i] = fk + np.linalg.norm(np.maximum(0, Population['c'][i, :]), NormType) + np.linalg.norm(
...
def custom_obj_function(x):
    f1 = x[0]**2 + x[1]**2
    f2 = (x[0] - 1)**2 + (x[1] - 1)**2
    return np.array([f1, f2])

def custom_constraints(x):
    c = [x[0]**2 + x[1] - 1.5]  # Two inequality constraints
    ceq = [x[0] - x[1] - 0.5]  # Two equality constraints
    return np.array(c), np.array(ceq)

Problem = {
    'ObjFunction': custom_obj_function,
    'Variables': 2,
    'Objectives': 2,
    'LB': np.array([0, 0]),
    'UB': np.array([1, 1]),
