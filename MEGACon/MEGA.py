...
def init_population(Problem, InitialPopulation, Size, Conflag, CTol, CeqTol, NormType, NormCons, *varargs):
    Population = {
        ...
        'c': np.zeros((Size, 1)),  # Assuming one inequality constraint
        'ceq': np.zeros((Size, 1)),  # Assuming one equality constraint
...
def con_eval(Problem, x, *varargs):
    ...
    try:
        ...
        ceq = ceq if ceq is not None else 0
    except Exception as e:
        raise ValueError(
...
def draw_illustration(Problem, X, Y, F1, F2, *varargs):
    ...
    # First subplot: 3D surface plot of the objective function
    ax1 = plt.subplot(2, 2, 1, projection='3d')
    ax1.set_title('Objective function')
    xx, yy = np.meshgrid(np.linspace(Problem['LB'][0], Problem['UB'][0], 80), np.linspace(Problem['LB'][1], Problem['UB'][1], 80))
    ...
    # Third subplot: Objective space plot
    ax3 = plt.subplot(2, 2, 3)
    ax3.plot(z[:, :, 0].flatten(), z[:, :, 1].flatten(), 'b.')
    ax3.set_xlabel('f1(x)')
    ax3.set_ylabel('f2(x)')

    # Fourth subplot: Pareto front plot
...
def ZDT1_con(x):
    c = [x[0]**2 - 0.5, x[2]-1]  # inequality constraint
    ceq = [x[0] + x[1] - 0.5]  # equality constraint
    return np.array(c), np.array(ceq)

# Problem definition for ZDT1
Problem = {
    'ObjFunction': ZDT1,
    'Variables': 2,
    'Objectives': 2,
...
# Initial population (randomly generated for demonstration)
initial_population = [
    np.array([0.1, 0.4])

]

# for _ in range(5, Options['PopSize']):
#     initial_population.append(np.random.rand(Problem['Variables']))

print(initial_population)
# Call the MEGAcon function
